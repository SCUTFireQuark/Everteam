...
- (void)loadMemoAction
{
    ...
    for (int i=0; i<self.displays.count; i++) {
        //一条便签
        int max = self.displays.count*100+10;
        UIImageView* iv = [[UIImageView alloc]initWithFrame:CGRectMake(30,max-(i+1)*100, 270, 90)];
        iv.tag = i;
        ...
        //放大button
        UIButton *largebtn = [[UIButton alloc]initWithFrame:CGRectMake(20, 10, 280, 90)];
...
-(void)updateUI{
    for (int i=0; i<self.ivs.count; i++) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            iv.frame = CGRectMake(20,10+i*100, 280, 90);
        }];
...
- (void)deleteMemo:(UIButton*)sender
{
    ...
    UIAlertView *av = [[UIAlertView alloc]initWithTitle:@"亲！！草你妹" message:@"你确定要草么？" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"要草哦", nil];
    [av show];
...
- (void)deleteTheme:(UIButton*)sender
{
    ...
    for (int i=sender.tag; i<= _btns.count; i++) {
        if (i == sender.tag) {
            ...
            [obj setFrame:CGRectMake(20,10+(i-1)*100, obj.frame.size.width,obj.frame.size.height)];
        }
    }
...
- (IBAction)neMemo:(id)sender {
    
    
    NSLog(@"newMemo");
    
    UIImageView* iv = [[UIImageView alloc]initWithFrame:CGRectMake(29,10, 270, 300)];
    iv.image = [UIImage imageNamed:@"5.png"];
    [self.ivs addObject:iv];
    
    self.summaryTextField=[[UITextField alloc] initWithFrame:CGRectMake(5, 25, 250,20)];
    self.detailsTextView=[[UITextView alloc]initWithFrame:CGRectMake(5, 55, 250,180)];
    self.summaryTextField.backgroundColor=[UIColor blueColor];
    self.detailsTextView.backgroundColor=[UIColor blueColor];
    ...
    for (NSInteger i=self.memoBtns.count-1; i>=0; i--) {
        UIButton *btn = [self.memoBtns objectAtIndex:i];
        UIImageView *iv2 = (UIImageView *)btn.superview;
        CGRect newPos = CGRectMake(iv2.frame.origin.x, iv2.frame.origin.y+300, iv2.frame.size.width, iv2.frame.size.height);
            [self moveMent:iv2 initialFrame:iv2.frame FinalFrame:newPos];
        
    }
    [self.UIMemoIV setContentSize:CGSizeMake(self.UIMemoIV.contentSize.width, self.UIMemoIV.contentSize.height+300)];
    ...
    memodeletebtn.frame = CGRectMake(120, -8, 21, 42);
    [memodeletebtn setBackgroundImage:[UIImage imageNamed:@"3.png"] forState:UIControlStateNormal];
...
-(void) completeNewMemo:(UIButton *)sender{
     ...
     sender.frame = CGRectMake(120, -8, 21, 42);
