...
@property(strong, nonatomic) UIGestureRecognizer *keyboardGesture;
...
- (void)panAction:(UILongPressGestureRecognizer *)sender
{
    ...
    if(((BQMemoView *)(sender.view)).frame.size.height == self.height){
        ...
        CGFloat distance = [height intValue] - memoView.frame.size.height;
        
        CGRect newPos = CGRectMake(memoView.frame.origin.x, memoView.frame.origin.y, memoView.frame.size.width, [height intValue]);
        [self memoMovement:memoView initialFrame:memoView.frame FinalFrame:newPos];
        
        [self moveMemosBelowForDistance:distance theView:memoView];
        
        NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:0.3 target:memoView selector:@selector(changeMemoState) userInfo:nil repeats:NO];
        ...
        self.keyboardGesture = [[UITapGestureRecognizer alloc]initWithTarget:memoView action:@selector(keyboardMissBoth:)];
        [self.memoPanelView addGestureRecognizer:self.keyboardGesture];
...
- (void)completeCreateMemo:(UIButton *)sender
{
    ...
    if (memoView.summaryTextField.text.length == [shortestSummary intValue]) {
        ...
        //移除动作，否则会出错
        [self.memoPanelView removeGestureRecognizer:self.keyboardGesture];
        // [self loadMemo];
    }
    self.pressDown = NO;
    [self.memoCreateButton setUserInteractionEnabled:YES];
...
- (IBAction)completeEditMemo:(id)sender
{
    ...
    BOOL isSummaryDuplicate = [self.memoBoard isDataBaseContainMemoWithSummary:memoView.summaryTextField.text exceptOrder:memoOrder];
    if (isSummaryDuplicate) {
        UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:summaryDuplicate message:message delegate:self cancelButtonTitle:cancel otherButtonTitles: nil];
        [alertView show];
        return;
    ...
    if (memoView.summaryTextField.text.length == [shortestSummary intValue]) {
        ...
        [self changeMemoWidgetSize:memoView.memoButton];
...
- (void)moveMemosBelowForDistance:(CGFloat)distance theView:(BQMemoView *)selectedView
{
    if (distance > 0) {
        for (NSInteger i=self.memoViewArray.count-1; i>=0; i--) {
            BQMemoView *memoView = [self.memoViewArray objectAtIndex:i];
            if (memoView.frame.origin.y <= selectedView.frame.origin.y) {
                break;
            }
            CGRect newPos = CGRectMake(memoView.frame.origin.x, memoView.frame.origin.y+distance, memoView.frame.size.width, memoView.frame.size.height);
            [self memoMovement:memoView initialFrame:memoView.frame FinalFrame:newPos];
    ...
    if (distance < 0) {
        NSInteger i;
        for (i=1; i<self.memoViewArray.count; i++) {
            BQMemoView *memoView = [self.memoViewArray objectAtIndex:i];
            if (memoView.frame.origin.y >= selectedView.frame.origin.y) {
                break;
            }
        }
        for ( ; i<self.memoViewArray.count; i++) {
            BQMemoView *memoView = [self.memoViewArray objectAtIndex:i];
            CGRect newPos = CGRectMake(memoView.frame.origin.x, memoView.frame.origin.y+distance, memoView.frame.size.width, memoView.frame.size.height);
            [self memoMovement:memoView initialFrame:memoView.frame FinalFrame:newPos];
        }
...
{
    for (BQMemoView *memoView in self.memoViewArray) {
        for (UIGestureRecognizer *recognizer in memoView.gestureRecognizers) {
            [memoView removeGestureRecognizer:recognizer];
        }
    }
...
{
    UIButton *themeButton = (UIButton *)[sender superview];
    NSNumber *shift = [self.memoModel.themeBtn objectForKey:@"shift"];
    NSNumber *height = [self.memoModel.themeBtn objectForKey:@"height"];
    int shitfdis = [shift intValue] + [height intValue], counter = -1;
    
    NSString *themeName = themeButton.titleLabel.text;
    [themeButton removeFromSuperview];
    
    for (UIButton *button in self.themeButtonArray) {
        if (button.frame.origin.y > themeButton.frame.origin.y) {
            CGRect newPos = CGRectMake(button.frame.origin.x, button.frame.origin.y - shitfdis, button.frame.size.width, button.frame.size.height);
            [self memoMovement:button initialFrame:button.frame FinalFrame:newPos];
        } else {
            counter ++;
        }
    }
    
    [self.themeButtonArray removeObjectAtIndex:counter];
    [self.themeDeleteButtonArray removeObjectAtIndex:counter];
    [self.memoBoard.themeArray removeObjectAtIndex:counter];
    
    [self.memoBoard removeThemeInDataBaseWithThemeName:themeName];
    [self.memoBoard removeAllMemosInTheme:themeName];
    
    if ([self.themeButtonArray count] == 1) {
        [self.themeCreateButton setUserInteractionEnabled:YES];
        [self.memoCreateButton setUserInteractionEnabled:YES];
        [self.themeCreateView setUserInteractionEnabled:YES];
        for (UIButton *themeDeletebtn in self.themeDeleteButtonArray) {
